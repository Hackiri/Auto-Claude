...
Usage:
    python tests/e2e/e2e_merge_tracking_test.py
"""
...
# Get project root (this file is in tests/e2e/)
...
def create_test_merge_entry():
    """Create a test merge entry JSON file matching MergeHistoryEntry format"""
    now = datetime.now()
    # Merge ID format: YYYYMMDD-HHMMSS-taskid (expected by _get_merge_file_path)
    merge_id = now.strftime("%Y%m%d-%H%M%S") + "-test-spec"

    # This matches the MergeHistoryEntry.to_dict() format
    merge_data = {
        "merge_id": merge_id,
        "task_id": "test-spec",
        "spec_name": "E2E Test Spec",
        "source_worktree": "/tmp/test-worktree",
        ...
        "success": True,
        "error_message": None,
        "pre_merge_commit": "abc123def455",
        "merge_commit": "abc123def456",
        "files_changed": ["existing_file.py"],
        "files_added": ["test_file_1.py", "test_file_2.py"],
        "files_deleted": [],
        "total_conflicts": 1,
        ...
        "conflicts_resolved": [
            {
                ...
                "ai_reasoning": "AI resolved the conflict",
                "tokens_used": 1500,
...
def test_merge_history_tracker():
    ...
    try:
        ...
        # Create year-month subdirectory based on merge_id format (YYYYMMDD-HHMMSS-taskid)
        # MergeHistoryTracker._get_merge_file_path expects: merge_id[:4] + "-" + merge_id[4:6]
        ...
        merge_file = year_month_dir / f"{merge_id}.json"
        with open(merge_file, 'w') as f:
            json.dump(test_entry, f, indent=2)

        print_success(f"Created merge entry: {merge_file}")

        # Update or create index in MergeHistoryTracker format
        # Index stores just merge_ids as strings, not full objects
        index_file = merge_dir / "index.json"
        if index_file.exists():
            with open(index_file, 'r') as f:
                index_data = json.load(f)
        else:
            index_data = {"merges": [], "last_updated": None}

        # Add merge_id to index (just the string, not an object)
        if merge_id not in index_data["merges"]:
            index_data["merges"].append(merge_id)
        ...
        if retrieved["success"] != test_entry["success"]:
            print_error(f"Success mismatch: {retrieved['success']} != {test_entry['success']}")
            return False

        print_success("Data integrity verified")
        print_success(f"Merge entry created successfully: {merge_id}")

        # Store merge_id for use in subsequent tests
        global test_merge_id
        test_merge_id = merge_id
...
# Global variable to store the test merge ID across tests
test_merge_id = None
...
def test_cli_commands():
    ...
    global test_merge_id
    if not test_merge_id:
        print_error("No test_merge_id available. Test 1 must pass first.")
        return False
    ...
    try:
        ...
        # Verify our test merge appears in the list
        if test_merge_id in result.stdout or "E2E Test Spec" in result.stdout:
            print_success(f"Test merge found in list output")
        print(result.stdout[:500])  # Print first 500 chars

        # Test show-merge
        print_info(f"Testing: python merge_history_cli.py show-merge {test_merge_id}")
        result = subprocess.run(
            [sys.executable, str(cli_script), "show-merge", test_merge_id],
            cwd=str(project_root),
            capture_output=True,
            text=True,
            timeout=10
    ...
    try:
        ...
        try:
            ...
            if not response.get("success"):
                print_error(f"JSON API returned error: {response.get('error', 'unknown')}")
                return False

            merges = response.get("data", [])
            print_success(f"JSON API returned {len(merges)} merge(s)")

            # Verify merge data structure
            if merges:
                merge = merges[0]
                required_fields = ["merge_id", "task_id", "spec_name", "success", "files_changed"]
                ...
                print_error("No merges returned by JSON API list")
                return False
        ...
        # Test get command
        print_info(f"Testing: python merge_history_json.py get {project_root} {test_merge_id}")
        result = subprocess.run(
            [sys.executable, str(json_script), "get", str(project_root), test_merge_id],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def cleanup_test_data():
    ...
    try:
        ...
        if merge_dir.exists() and test_merge_id:
            # Calculate the year-month directory from the merge_id
            year_month = f"{test_merge_id[:4]}-{test_merge_id[4:6]}"
            year_month_dir = merge_dir / year_month

            # Remove the specific test merge file
            if year_month_dir.exists():
                test_file = year_month_dir / f"{test_merge_id}.json"
                if test_file.exists():
                    test_file.unlink()
                    print_success(f"Removed: {test_file.name}")

                # Also remove any files containing "-test-spec" (our test pattern)
                for f in year_month_dir.glob("*-test-spec.json"):
                    f.unlink()
                    print_success(f"Removed: {f.name}")

                # Remove empty directories
                if year_month_dir.exists() and not any(year_month_dir.iterdir()):
                    year_month_dir.rmdir()
                    print_success(f"Removed empty directory: {year_month_dir.name}")

            # Update index.json to remove test entries
            index_file = merge_dir / "index.json"
            if index_file.exists():
                with open(index_file, 'r') as f:
                    index_data = json.load(f)

                # Filter out test entries (those ending with -test-spec)
                # Index stores merge_ids as strings
                original_count = len(index_data.get("merges", []))
                index_data["merges"] = [
                    m for m in index_data.get("merges", [])
                    if not (isinstance(m, str) and m.endswith("-test-spec"))
                ]

                if len(index_data["merges"]) < original_count:
                    with open(index_file, 'w') as f:
                        json.dump(index_data, f, indent=2)
