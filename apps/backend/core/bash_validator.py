...
class BashValidationResult:
    ...
    allowed: bool
    reason: Optional[ValidationReason] = None
    subcommand_results: Optional[List[SubcommandResult]] = None
...
def check_control_characters(command: str) -> Optional[ValidationReason]:
    """
...
def check_command_against_patterns(command: str, patterns: List[CompiledPattern]) -> bool:
    """
...
def _validate_node_ast(node, patterns: List[CompiledPattern], results: List[SubcommandResult]) -> Optional[ValidationReason]:
    """
    ...
    if kind == 'redirect':
        return ValidationReason(
            ...
            details=f"Found redirect operator"
        )
...
def _validate_with_regex_fallback(command: str, patterns: List[CompiledPattern]) -> BashValidationResult:
    """
